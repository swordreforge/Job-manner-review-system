...
// 【修复核心1】：增加了一个 wrapper。用来测量固定的真实坐标，把动画赋值给内部的 card
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const wrappersRef = useRef<HTMLElement[]>([]); // 新增：保存 wrapper 的引用
  ...
  // 获取坐标的计算将不会再受到 translateY 变化的影响
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    wrappersRef.current.forEach((wrapper, i) => {
      ...
      // 【修复核心3】：测量 wrapper 的 top (静态，绝不会闪烁)
      ...
      if (hasChanged) {
        ...
        // 【修复核心4】：将计算出来的 transform 赋予卡片内部 (wrapper 不动)
